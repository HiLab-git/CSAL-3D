...
class GaussianNoised(MapTransform):
    ...
    def __call__(self, data):
        if np.random.uniform() < self.prob:
            for k in self.keys:
                data[k] = MetaTensor(augment_gaussian_noise(
                    data[k].numpy(),
                    self.noise_variance,
                    self.p_per_channel,
                    self.per_channel))
...
class GaussianBlurd(MapTransform):
    ...
    def __call__(self, data):
        if np.random.uniform() < self.prob:
            for k in self.keys:
                data[k] = MetaTensor(augment_gaussian_blur(
                    data[k].numpy(),
                    self.blur_sigma,
...
class BrightnessMultiplicatived(MapTransform):
    ...
    def __call__(self, data):
        if np.random.uniform() < self.prob:
            for k in self.keys:
                data[k] = MetaTensor(augment_brightness_multiplicative(
                    data[k].numpy(),
...
class ContrastAugmentationd(MapTransform):
    ...
    def __call__(self, data):
        if np.random.uniform() < self.prob:
            for k in self.keys:
                data[k] = MetaTensor(augment_contrast(
                    data[k].numpy(),
...
class SimulateLowResolutiond(MapTransform):
    ...
    def __call__(self, data):
        if np.random.uniform() < self.prob:
            for k in self.keys:
                data[k] = MetaTensor(augment_linear_downsampling_scipy(
                    data[k].numpy(),
...
class Gammad(MapTransform):
    ...
    def __call__(self, data):
        if np.random.uniform() < self.prob:
            for k in self.keys:
                data[k] = MetaTensor(augment_gamma(
                    data[k].numpy(),
                    self.gamma_range,
                    self.invert_image,
                    per_channel=self.per_channel,
