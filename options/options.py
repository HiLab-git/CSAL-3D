...
class Options():
    ...
    def __init__(self):
        ...
        parser.add_argument('--RS_seed', type=int, default=0, help='seed for randomly select labeled data')
        ...
        parser.add_argument('--resolution', type=str, default='1mm', help='data resolution')
        parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints', help='networks are saved here')

        # Supply for Swin-UNETR
        parser.add_argument("--local_rank", type=int, default=0, help="local rank")

        # Supply for SwinMM
        parser.add_argument("--mask_ratio", default=0.3, type=float, help="mask ratio for MAE pretraining")
        parser.add_argument("--window_size", default=16, type=int, help="window size for MAE pretraining")
        parser.add_argument("--mutual_learning_on_more_view", default=True, type=bool, help="also use rotate for mutual learning")
        parser.add_argument("--norm_pix_loss", default=True, type=bool, help="normalize before compute reconstruction loss")

        # Load pre-trained weights
        parser.add_argument('--load_ckpt', default='/media/ubuntu/zhuning/COLosSAL-3D/checkpoints/Spleen/FPS_3/best_model.pth', help='whether to use the pre-trained model weights/checkpoint. Defaults to False')
        ...
        # CNN
        parser.add_argument('--nid', type=int, default=1, help="network id [0: DI2IN | 1: UNet (monai) | 2: nnFormer | 3: generic UNet | 999: fine-tune]")
        ...
        parser.add_argument('--dropout_path_rate', type=float, default=0.2, help='dropout_path_rate')
        parser.add_argument('--do_ds', action='store_true', help='whether to use deep supervision. Defaults to False')

        # Transformer 
        parser.add_argument('--embedding_dim', type=int, default=48, help='dimension of embedding feature vector in nnFormer')
        ...
        # parser.add_argument('--crop_size', default=(10, 5, 5, 5), type=int, help='cropped patch size')
        ...
        parser.add_argument('--val_iterations', type=int, default=2000, help='number of iterations per validation phase')
        parser.add_argument('--skip_val_iterations', type=int, default=10000, help='number of iterations to skip validation phases')
        parser.add_argument('--early_stop', type=int, default=2000, help='number of epochs without improvements for early stop')
        parser.add_argument('-track', '--display_per_iter', default=True, action='store_true', help='whether to display training loss and validation dsc per iteration')
        ...
        parser.add_argument('--save_output', default=True, action='store_true', help='whether to save predictions in an output directory. Defaults to False')
